...
public class BaiduNavigationActivity extends Activity {
	...
	private final String TAG = BaiduNavigationActivity.class.getName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null) {
			...
			if (bundle != null) {
				mBNRoutePlanNode = (BNRoutePlanNode) bundle.getSerializable(DemoRoutePlanListener.ROUTE_PLAN_NODE);
			}
		}
		//显示自定义图标
		if (hd != null) {
					...
					if (msg.what == MSG_SHOW) {
						...
						if (mBNRoutePlanNode != null) {
							BNRouteGuideManager.getInstance().resetEndNodeInNavi(
									new BNRoutePlanNode(mBNRoutePlanNode.getLongitude(), mBNRoutePlanNode.getLatitude(), mBNRoutePlanNode.getName(), null, CoordinateType.BD09LL));
						}
					}
				};
			};
